...
| Offline activation | Supported | Planned | Planned | Partially portable | RSA and request parsing are reusable. Phase 1i freezes direct legacy machine-ID binding and proves there is no identity-version metadata; stable native HWID and packaged public-key validation are not complete. |
| HWID | Supported | Planned | Planned | Windows-only | Phase 1i freezes Windows registry value + complete volume stdout + always-included generic fallback, exact serialization/hash, and failure behavior using synthetic fixtures. Compatibility/privacy risks are in `docs/hardware-identity-audit.md`; no provider or native identity is implemented. |
