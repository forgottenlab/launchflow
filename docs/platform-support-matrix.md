...
| Diagnostics | Supported | Planned | Planned | Partially portable | Redaction and bounded text generation are reusable; `Windows:` and `%USERPROFILE%` labels remain Windows-centric. Folder opening now has a platform boundary, but this does not implement native Linux/macOS integration. |
