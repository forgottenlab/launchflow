...
| `tools/check_desktop_integration_smoke.py` | Any source host with injected platform/opener fixtures | Windows `os.startfile(str(path))`; non-Windows historical no-op | Yes | `platform-neutral`, `windows-only` | Keep API/order/error/no-side-effect fixtures neutral; add native open-directory backends only in later supported phases. |
